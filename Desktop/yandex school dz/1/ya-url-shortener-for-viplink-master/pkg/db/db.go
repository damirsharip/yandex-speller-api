...
func (db *Database) Save(ctx context.Context, shortSuffix, longLink, secretKey string, expirationDate time.Time) error {
	_, err := db.client.ExecContext(ctx, saveRequest, shortSuffix, longLink, secretKey, expirationDate)
	return err
}
...
func (db *Database) SelectBySuffix(ctx context.Context, shortSuffix string) (*Link, error) {
	...
	var link Link
	err := row.Scan(&link.ShortSuffix, &link.Link, &link.SecretKey, &link.Clicks, &link.ExpirationDate)
	switch {
