...
func (c *Client) CheckTexts(ctx *gin.Context, texts []string) (models.YandexResponse, error) {
	res, err := c.send(ctx, texts)

	return res, err
}

func (r *Client) send(ctx *gin.Context, postData []string) (models.YandexResponse, error) {
	//logger := logger.LoggerFromGinContext(ctx)
	var yr models.YandexResponse
